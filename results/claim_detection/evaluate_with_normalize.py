...
class PromptAgent(ClaimUnderstandingAgent):
    ...
    and overrides ONLY the prompt, so all arms share identical extraction logic."""
...
async def evaluate(split, n, prompts_to_run):
    ...
    # Every arm runs the same extraction logic, differing only by prompt.
    agents = {
        ...
        for name, agent in agents.items():
            try:
                predicted = await agent.detect_claims(
                    input_text, min_importance=0.0, normalize_detected=True
                )
    ...
    output_file = f"results/claim_detection/metrics_{split}_with_normalize.json"
